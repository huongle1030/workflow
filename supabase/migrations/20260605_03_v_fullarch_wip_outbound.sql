-- Full Arch WIP worklist for the "Pending Outbound" sub-tab.
--
-- The Pending Outbound tab previously read v_pending_outbound_triage, which only surfaces cases
-- that already have a pending_approval email draft (dr_outreach_attempts). Coordinators want the
-- tab to be the full worklist of Full-Arch work-in-progress cases instead -- a card per case
-- whether or not any outreach has been drafted/communicated yet.
--
-- Scope (confirmed with the user):
--   * Full Arch  = "Cases"."Business Unit" = 'Full Arch'
--   * WIP        = "Cases"."Case Status"   = 'WIP'
--
-- The view is shaped to match v_pending_outbound_triage so renderOutboundCard can render the rows
-- directly. A case that HAS a pending draft carries attempt_id/subject/body_html (so Approve/Edit/
-- Reject keep working); a case with no draft has attempt_id = NULL (the UI renders a read-only
-- card with Lookup + "Draft outreach email"). All cards still get the revenue/partner/comm/prefs
-- enrichments, so the tab is a complete worklist.
...
pend AS (   -- latest pending_approval draft per case, if any
