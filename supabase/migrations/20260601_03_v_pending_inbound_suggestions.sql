-- Part 4 — expose the suggestion columns in v_pending_inbound.
--
-- The three suggested_* columns are appended at the END of the select list:
-- CREATE OR REPLACE VIEW can only add columns after the existing ones (it cannot
-- reorder them), and column position is irrelevant to the front-end (selects by
-- name). Everything else is byte-for-byte the prior definition.
...
SELECT
  ...
  r.suggested_case_number,   -- NEW
  r.suggested_confidence,    -- NEW
  r.suggested_reasoning      -- NEW
