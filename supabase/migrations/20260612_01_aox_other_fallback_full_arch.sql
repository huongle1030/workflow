-- AOX-on-'Other' fallback for Full Arch classification.
--
-- Some cases that are really Full Arch are stored in the catch-all Business Unit 'Other' instead of
-- 'Full Arch', so they fall out of every "Full Arch only" gate (Pending Outbound / design approval,
-- parse enqueue). Fallback rule: when Business Unit = 'Other', treat the case as Full Arch if its
-- Hold Reason is an AOX hold (e.g. 'E-AOX-Hold for Doctor Design Approval' — any reason containing
-- 'AOX', which also covers the 'starts with E-AOX' case).
--
-- The rule is centralised in is_full_arch_case(business_unit, hold_reason) so every place that gates
-- on Full Arch shares one definition. Today that is two objects: the v_fullarch_wip_outbound view
-- (Pending Outbound / design approval worklist) and the enqueue_case_parse_from_comm() trigger.

CREATE OR REPLACE FUNCTION public.is_full_arch_case(p_business_unit text, p_hold_reason text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  -- True when the BU is literally 'Full Arch', OR (fallback) the BU is the catch-all 'Other' but the
  -- Hold Reason marks it as AOX. ILIKE '%AOX%' is case-insensitive and also matches 'E-AOX...'.
  SELECT p_business_unit = 'Full Arch'
      OR (p_business_unit = 'Other' AND COALESCE(p_hold_reason ILIKE '%AOX%', false));
$$;

GRANT EXECUTE ON FUNCTION public.is_full_arch_case(text, text) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------------------------------
-- v_fullarch_wip_outbound: swap the `"Business Unit" = 'Full Arch'` gate for is_full_arch_case(...).
-- Body is otherwise identical to 20260610_10_fullarch_wip_outbound_catchall_drafts.sql.
-- ---------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW public.v_fullarch_wip_outbound AS
 WITH wip AS (
         SELECT c_1."Case Status",
            ...
            c_1."Reschedule Flag"
           FROM "Cases" c_1
          WHERE public.is_full_arch_case(c_1."Business Unit", c_1."Hold Reason")
             ...
             OR (c_1."Case Number" IN ( SELECT q.case_number
 ...
 SELECT pend.attempt_id,
   ...
   FROM wip c
     ...
     LEFT JOIN LATERAL ( SELECT r2.cc_recipients,
         ...
         LIMIT 1) lr ON true
  WHERE COALESCE(ps.show_in_pending_outbound, true) OR pend.attempt_id IS NOT NULL;

GRANT SELECT ON public.v_fullarch_wip_outbound TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------------------------------
-- enqueue_case_parse_from_comm(): same Full Arch gate, same fallback. Body otherwise identical to
-- 20260607_02_case_parse_queue.sql.
-- ---------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE c."Case Number" = NEW.case_number
      AND public.is_full_arch_case(c."Business Unit", c."Hold Reason")
