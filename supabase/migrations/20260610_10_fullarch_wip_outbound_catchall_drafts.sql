-- Make Pending Outbound the catch-all destination for EVERY pending draft. Previously a draft only
-- surfaced if its case was a Full Arch design-approval case (WIP/Hold). Classification drafts created
-- on Other/Removable/Restorative or Shipped cases were composed but never shown. Two changes:
--   1. The driving set (wip CTE) now also includes any case that has a pending_approval attempt,
--      regardless of business unit / status / step.
--   2. The final visibility filter no longer suppresses a case that has a draft (a parse-state
--      show_in_pending_outbound=false can hide an empty worklist card, but never a real draft).
-- Empty (no-draft) cards are still limited to the Full Arch design-approval worklist, so the tab
-- isn't flooded with draft-less cases from every business unit.
CREATE OR REPLACE VIEW public.v_fullarch_wip_outbound AS
 WITH wip AS (
         SELECT c_1."Case Status",
           ...
           FROM "Cases" c_1
          WHERE (
                  c_1."Business Unit" = 'Full Arch'::text
