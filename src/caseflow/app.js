...
export async function reload() {
  try { cases = await Data.loadAll(); loaded = true; }
...
function renderQueue(mode) {
  ...
  const list = cases.filter(c => stages.includes(c.stage));
  return `<div style="margin-bottom:14px"><div style="font-size:16px;font-weight:600">${MODES[mode].title}</div></div>` +
    (list.length ? tblWrap(list, mode) : emptyMsg('No cases pending'));
}

// ── Design Team queue (5 sub-tabs by phase) ─────────────────────────
let designTab = 'design';
const DESIGN_SUBTABS = [
  ['design', 'Design', ['Design Check']],
  ['outsource', 'Outsource', ['Outsourcing']],
  ['qc', 'QC', ['QC']],
  ['rework', 'Rework', ['QC Failed - Rework', 'QC Failed - Resend']],
  ['complete', 'Complete', ['Complete']],
];
function renderDesignQueue() {
  const tabs = DESIGN_SUBTABS.map(([k, label, st]) => {
    const n = cases.filter(c => st.includes(c.stage)).length;
  ...
  const list = cases.filter(c => active[2].includes(c.stage));
  const head = `<div style="font-size:16px;font-weight:600;margin-bottom:12px">Design Team</div>`;
  return head + `<div class="de-tabs">${tabs}</div>` + (list.length ? tblWrap(list, 'design') : emptyMsg('No cases here'));
}
...
function renderDataEntryQueue() {
  ...
  const main = de.filter(c => !c.deHold), models = de.filter(c => c.deHold === 'models'), missing = de.filter(c => c.deHold === 'missing');
  const tabs = [['main', 'Data Entry', main.length], ['models', 'Waiting for physical models', models.length], ['missing', 'Waiting for missing information', missing.length]]
    .map(t => `<button class="de-tab${deTab === t[0] ? ' active' : ''}" onclick="CF.setDeTab('${t[0]}')">${t[1]}<span class="de-tab-count">${t[2]}</span></button>`).join('');
  const list = deTab === 'models' ? models : (deTab === 'missing' ? missing : main);
  ...
  return head + `<div class="de-tabs">${tabs}</div>` + (list.length ? tblWrap(list, 'dataentry') : emptyMsg('No cases here'));
}
...
function buildAoxReviewChecklist(caseId) {
  ...
  function fileDrop(name) {
    ...
    return `<div class="form-group" style="margin-bottom:10px"><label>${name}</label><div class="upload-zone" ondragover="CF.aspDragOver(event)" ondragleave="CF.aspDragLeave(event)" ondrop="CF.aspDrop(event,'${caseId}','${key}')" onclick="CF.aspPick('${caseId}','${key}')"><div class="upload-icon"><i class="ti ti-upload"></i></div><div class="upload-text">Drag files here or click to attach</div></div>${list}</div>`;
  }
...
async function aspDrop(ev, id, key) { ev.preventDefault(); ev.stopPropagation(); ev.currentTarget.classList.remove('drag-over'); const fl = ev.dataTransfer && ev.dataTransfer.files; if (fl && fl.length) await uploadReviewFiles(id, key, Array.from(fl)); }
function aspPick(id, key) { pickFiles(async files => { await uploadReviewFiles(id, key, files); }); }
...
function buildASPRecipe(a) { if (a.cat !== 'ASP') return ''; if (a.aspSub === 0 && isSet(a.aspType)) { const matSet = isSet(a.aspRcpMat); let inner = ''; if (matSet && a.aspRcpMat === 0) inner = `<span class="sec-label">Select option</span>${optRows('ASP_RCP_ZIRC', isSet(a.aspRcpZirc) ? a.aspRcpZirc : -1)}`; if (matSet && a.aspRcpMat === 1) { const pmmaSet = isSet(a.aspRcpPmmaType); inner = `<span class="sec-label">PMMA type</span>${typeGrid(['Analog Milled', 'Verification Jig'], pmmaSet ? a.aspRcpPmmaType : -1, 'aspRcpPmmaType')}${pmmaSet && a.aspRcpPmmaType === 1 ? `<span class="sec-label">VJ type</span>${optRows('ASP_RCP_VJ', isSet(a.aspRcpVJ) ? a.aspRcpVJ : -1)}` : ''}`; } return `<div class="recipe-block"><div class="recipe-block-title"><i class="ti ti-chef-hat" style="font-size:14px"></i> Recipe Selection</div><span class="sec-label" style="margin-top:0">Material</span>${typeGrid(['Zirconia', 'PMMA'], matSet ? a.aspRcpMat : -1, 'aspRcpMat')}${inner}</div>`; } if (a.aspSub === 1 && isSet(a.aspType)) { return `<div class="recipe-block"><div class="recipe-block-title"><i class="ti ti-chef-hat" style="font-size:14px"></i> Recipe Selection</div><span class="sec-label" style="margin-top:0">Select option</span>${optRows('ASP_RCP_LFX', isSet(a.aspRcpLfx) ? a.aspRcpLfx : -1)}</div>`; } return ''; }
function buildSKDRecipe(a) { if (a.cat !== 'SKD' || !isSet(a.skdFinal)) return ''; const matSet = isSet(a.skdRcpMat); let inner = ''; if (matSet && a.skdRcpMat === 0) inner = `<span class="sec-label">Select option</span>${optRows('SKD_RCP_ZIRC', isSet(a.skdRcpZirc) ? a.skdRcpZirc : -1)}`; if (matSet && a.skdRcpMat === 1) inner = `<div class="opt-row sel" style="pointer-events:none;opacity:.85"><input type="radio" checked><span class="opt-row-label">Analog Milled</span></div>`; if (matSet && a.skdRcpMat === 2) inner = `<span class="sec-label">VJ type</span>${optRows('SKD_RCP_VJ', isSet(a.skdRcpVJ) ? a.skdRcpVJ : -1)}`; return `<div class="recipe-block"><div class="recipe-block-title"><i class="ti ti-chef-hat" style="font-size:14px"></i> Recipe Selection</div><span class="sec-label" style="margin-top:0">Select material / type</span><div class="type-grid-3">${['Zirconia', 'PMMA', 'Verification Jig'].map((txt, i) => `<div class="opt-row${a.skdRcpMat === i ? ' sel' : ''}" data-aox-field="skdRcpMat" data-aox-val="${i}"><input type="radio" ${a.skdRcpMat === i ? 'checked' : ''}><span class="opt-row-label">${txt}</span></div>`).join('')}</div>${inner}</div>`; }
...
function buildAox(caseId) {
  ...
  let catSub = '';
  if (a.cat === 'CC') { const ccTypeSet = isSet(a.ccType), optKey = a.ccType === 1 ? 'CC_CONT' : 'CC_NEW'; catSub = `<span class="sec-label">Case type</span>${typeGrid(['New Case', 'Continuation'], isSet(a.ccType) ? a.ccType : -1, 'ccType')}${ccTypeSet ? `<span class="sec-label">Select option</span>${optRows(optKey, isSet(a.ccOption) ? a.ccOption : -1)}` : ''}${buildCCRecipe(a)}`; }
  if (a.cat === 'ASP') { const aspSubSet = isSet(a.aspSub), aspTypeSet = isSet(a.aspType); catSub = `<span class="sec-label">Product type</span>${typeGrid(['Zirconia / PMMA', 'Locator Fixed (LFX)'], aspSubSet ? a.aspSub : -1, 'aspSub')}${a.aspSub === 0 ? `<span class="sec-label">Case type</span>${optRows('ASP_ZP_TYPE', aspTypeSet ? a.aspType : -1)}${aspTypeSet ? `<span class="sec-label">Select option</span>${optRows('ASP_ZP_FINAL', isSet(a.aspFinal) ? a.aspFinal : -1)}` : ''}${buildASPRecipe(a)}` : ''}${a.aspSub === 1 ? `<span class="sec-label">Case type</span>${optRows('ASP_LFX_TYPE', aspTypeSet ? a.aspType : -1)}${aspTypeSet ? `<span class="sec-label">Select option</span>${optRows('ASP_LFX_FINAL', isSet(a.aspFinal) ? a.aspFinal : -1)}` : ''}${buildASPRecipe(a)}` : ''}`; }
  if (a.cat === 'SKD') { const skdTypeSet = isSet(a.skdType), skdFinalSet = isSet(a.skdFinal); catSub = `<span class="sec-label">Case type</span>${typeGrid(['New Case', 'Continuation'], skdTypeSet ? a.skdType : -1, 'skdType')}${skdTypeSet ? `<span class="sec-label">Select option</span>${optRows('SKD_FINAL', skdFinalSet ? a.skdFinal : -1)}` : ''}${buildSKDRecipe(a)}`; }
  ...
  const addonSub = a.addon ? `<div style="padding-left:4px;margin-top:6px">${optRows('ADDON', isSet(a.addonSel) ? a.addonSel : -1)}</div>` : '';
...
function reviewSummaryPanel(c) {
  ...
  if (acct === 'tri') {
    ...
    if (r.drApproval) dn += pair('Doctor design approval', r.drApproval === 'yes' ? 'Yes — Required' : 'No — Not required');
    ...
    let ds = '';
    if (r.aspSpec) ASP_SPEC_ITEMS.forEach(it => { if (r.aspSpec[it[0]]) ds += pair(it[1], yn(r.aspSpec[it[0]])); });
...
function renderCaseDetail(c) {
  ...
  const sfH = c.scanFiles && c.scanFiles.length ? c.scanFiles.map(f => `<div class="file-item"><i class="ti ti-scan"></i><span class="file-name">${esc(f.name)}</span><span class="file-size">${f.size}</span></div>`).join('') : '<span style="font-size:12px;color:var(--color-text-tertiary)">No scan files yet</span>';
  const a = getA(c.id); const r = getR(c.id);
  let sp = '', reviewInfoPanel = '';

  if (c.stage === 'Data Entry') {
    sp = `<div class="panel"><div class="panel-title"><i class="ti ti-pencil"></i> Case Information</div>
    ...
    <div class="panel"><div class="panel-title"><i class="ti ti-paperclip"></i> ABS Attachments</div>
      <div class="upload-zone" onclick="CF.pick('entry')"><div class="upload-icon"><i class="ti ti-upload"></i></div><div class="upload-text">Click to attach files</div><div class="upload-hint">PDF, DOCX, ZIP, images</div></div>
  ...
  if (['Design Check', 'Outsourcing', 'QC', 'QC Failed - Rework', 'QC Failed - Resend', 'Complete'].includes(c.stage)) {
    ...
    sp = doctorReqsPanel(c) + reviewSummaryPanel(c) + outsourceRO + qcRO + sp + designChecklistPanel(c);
  }
...
function submitDataEntry(id) { saveDeFields(id); const c = getC(id); if (c) c.deHold = null; advanceStage(id, 'Review', 'Data submitted and sent to review', 'Data Entry Team'); }
...
function pickFiles(cb) { const inp = document.createElement('input'); inp.type = 'file'; inp.multiple = true; inp.style.display = 'none'; document.body.appendChild(inp); inp.onchange = () => { const files = Array.from(inp.files || []); document.body.removeChild(inp); if (files.length) cb(files); }; inp.click(); }
function pick(kind) {
...
export function initCaseFlow() {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (catBtn) { const cat = catBtn.dataset.aoxCat; a.cat = a.cat === cat ? null : cat; ['ccType', 'ccOption', 'rcpDigital', 'rcpOption', 'aspSub', 'aspType', 'aspFinal', 'aspRcpMat', 'aspRcpZirc', 'aspRcpPmmaType', 'aspRcpVJ', 'aspRcpLfx', 'skdType', 'skdFinal', 'skdRcpMat', 'skdRcpZirc', 'skdRcpVJ', 'engFinal', 'engSurgicalAddon', 'engDoctor', ...TRI_ALL].forEach(k => a[k] = null); rebuildAox(caseId); return; }
    ...
    if (tog) { const t = tog.dataset.aoxToggle; a[t] = !a[t]; if (t === 'addon' && !a[t]) a.addonSel = null; rebuildAox(caseId); }
  });
}

const CF = {
  openCase, goBack, setDeTab, setDesignTab, advanceStage,
  ...
  aspDragOver, aspDragLeave, aspDrop, aspPick, aspRemoveFile, setPF, setDesignNeeds, setDrApproval, saveReviewNotes,
  // doctor reqs
  saveDesignReqs, addDesignReq, removeDesignReq, toggleReqsAck,
  // review transitions
  passToDesign, passToScanning, showCoordReason, sendToCoordination,
  // design checklist
  ...
  // data entry
  toggleDeRush, submitDataEntry, holdDataEntry, pick,
  // new case
